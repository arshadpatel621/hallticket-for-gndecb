...
app.post(['/generate', '/api/generate'], upload.fields([{ name: 'excelFile', maxCount: 1 }, { name: 'logoFile', maxCount: 1 }]), async (req, res) => {
    try {
        ...
        // Use uploaded logo if present, otherwise use default logo.jpg
        const logoPath = req.files['logoFile'] ? req.files['logoFile'][0].path : path.join(__dirname, 'logo.jpg');
